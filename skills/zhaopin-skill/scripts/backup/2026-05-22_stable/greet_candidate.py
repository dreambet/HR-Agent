...
def find_candidate_on_page(page, candidate_name, candidate_info=None):
    ...
    result = page.evaluate('''(params) => {
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            result.matchedHints = matchedHints;
            result.unmatchedHints = unmatchedHints;
...
def has_next_page(page):
    """检查是否有下一页"""
    try:
        next_buttons = page.locator('button:has-text("下一页"), [class*="next"]')
                ...
                if select_result.get('success'):
                    ...
                    try:
                        ...
                        if js_result.get('success'):
                            ...
                            if 'AI招呼语' in page_text or '使用并发送' in page_text:
                                print(f"   ✅ 检测到招呼语设置对话框")
...
def main():
    ...
    # 如果没有提供关键词，使用上下文中的关键词
    if not args.keyword:
        if context:
            args.keyword = context.get('keywords', '')
            args.location = args.location or context.get('location', '')
            args.education = args.education or context.get('education', '')
            args.experience = args.experience or context.get('experience', '')
        else:
            print("\n警告：未提供搜索关键词，且没有可用的搜索上下文")
